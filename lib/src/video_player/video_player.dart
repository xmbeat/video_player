...
export 'package:video_player_android/src/video_player_platform_interface/video_player_platform_interface.dart'
    show DataSourceType, DurationRange, VideoFormat, VideoPlayerOptions;

export 'closed_caption_file.dart';
...
class VideoPlayerValue {
  ...
  //Used for last custom data source event detail.
  final Object? detail;
...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  VideoPlayerController.contentUri(Uri contentUri,
      {Future<ClosedCaptionFile>? closedCaptionFile, this.videoPlayerOptions, this.aesOptions = null})
  ...
  VideoPlayerController.customDataSource(String uri, StreamDataSource streamDataSource, 
    {this.videoPlayerOptions, this.aesOptions = null})
    :dataSource = uri,
      ...
      switch (event.eventType) {
        ...
        case VideoEventType.dataSourceOpen:
          ...
          _streamHandler!.open(dataSource, position: position, length: length).then((value){
            _videoPlayerPlatform.sendDataSourceOpenResponse(_textureId, value);
          }).catchError((e){
            _videoPlayerPlatform.sendDataSourceOpenResponse(_textureId, null, errorCode: 1);
          });
          break;
        case VideoEventType.dataSourceRead:
          int readLength = (event.detail as Map)['readLength'];
          _streamHandler!.read(readLength).then((value) {
            return _videoPlayerPlatform.sendDataSourceReadResponse(_textureId, value);
          }).catchError((e){
             _videoPlayerPlatform.sendDataSourceReadResponse(_textureId, null, errorCode: 1);
          });
          break;
        case VideoEventType.dataSourceClose:
          _streamHandler!.close().then((value){
            return _videoPlayerPlatform.sendDataSourceCloseResponse(_textureId);
          }).catchError((e){
            _videoPlayerPlatform.sendDataSourceCloseResponse(_textureId, errorCode: 1);
          });
          break;
...
class StreamHandler{
  ...
  Uint8List _readFromChunks(int length){
    Uint8List data = Uint8List(math.min(length, _totalChunkSizes));
    int pos = 0;
    int i = 0;
